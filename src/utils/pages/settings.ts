...
export class MemoryGraphSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const modelSetting = new Setting(containerEl)
			.setName('模型名称')
			.setDesc(this.modelsFetchSuccess
				? '从下拉列表中选择模型'
				: '输入要使用的模型名称（例如: claude-3-5-sonnet-20241022 或 gpt-4）');

		if (this.modelsFetchSuccess && this.availableModels.length > 0) {
			// 成功拉取到模型列表，显示下拉框
	 ...
	 */
	private async fetchModelsListIfReady(): Promise<void> {
		const settings = this.plugin.settings;

		// 检查必要的配置是否都已填写
		if (!settings.llmApiUrl || !settings.llmApiKey || !settings.llmApiType) {
			console.log('[Settings] 配置不完整，跳过拉取模型列表');
			return;
		}

		console.log('[Settings] 开始拉取模型列表...');
		...
		try {
			...
			if (result.success && result.models && result.models.length > 0) {
				console.log('[Settings] ✓ 成功拉取模型列表，模型数量:', result.models.length);
				...
				console.log('[Settings] ✗ 拉取模型列表失败:', result.error);
				this.availableModels = [];
				this.modelsFetchSuccess = false;
			}
		} catch (error) {
			console.error('[Settings] ✗ 拉取模型列表异常:', error);
			this.availableModels = [];
			this.modelsFetchSuccess = false;
		}
	}
